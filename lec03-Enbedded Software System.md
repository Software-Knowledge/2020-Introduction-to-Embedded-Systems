...
## 1.5. RISC vs. CISC
1. Complex instruction set computer (CISC): 复杂指令集计算机
   1. 许多寻址模式
   2. 多种操作
2. Reduced instruction set computer (RISC): 精简指令集计算机
   1. 加载/存储
   2. 可传递指令
...
## 1.12. 嵌入式微控制器（MCU，Microcontroller Unit）
1. 嵌入式微控制器就是将整个计算机系统的主要硬件集成到一块芯片中,芯片内部集成ROM/EPROM, RAM, 总线, 总线逻辑, 定时/计数器器, Watchdog, I/O, 串行口等各种必要功能和外设.
2. 特点：
   1. 一个系列的微控制器具有多种衍生产品;
...
## 1.17. 嵌入式SoC
1. 嵌入式SoC是追求产品系统最大包容的集成器器件。绝大多数系统构件都在一个系统芯⽚片内部。
2. 特点：
   1. 结构简洁
   2. 体积小、功耗小
   3. 可靠性高
